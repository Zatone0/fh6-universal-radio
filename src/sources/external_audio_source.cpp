...
float sample_to_float(const BYTE* p, WORD container_bits, WORD valid_bits, bool is_float) noexcept {
    ...
    switch (container_bits) {
        ...
        case 24: signed_value = read_i24_le(p); break;
        case 32: std::memcpy(&signed_value, p, sizeof(signed_value)); break;
        default: return 0.0f;
    }
    ...
    const auto scale = static_cast<double>(int64_t{1} << (valid_bits - 1u));
...
bool parse_input_format(const WAVEFORMATEX& fmt, InputFormat& out, std::string& error) {
    ...
    if (fmt.wFormatTag == WAVE_FORMAT_EXTENSIBLE &&
        ...
        out.valid_bits =
            ext.Samples.wValidBitsPerSample ? ext.Samples.wValidBitsPerSample : out.container_bits;
...
struct Resampler {
    ...
    void push_packet(const BYTE* data, UINT32 frame_count, DWORD flags) {
        ...
        const auto bytes_per_sample = static_cast<uint16_t>((fmt.container_bits + 7u) / 8u);
        ...
        for (UINT32 f = 0; f < frame_count; ++f) {
            ...
            if (!silent && data) {
                ...
                if (fmt.channels == 1) {
                    ...
                    l = sample_to_float(frame, fmt.container_bits, fmt.valid_bits, fmt.is_float);
                    r = sample_to_float(frame + bytes_per_sample, fmt.container_bits,
                                        fmt.valid_bits, fmt.is_float);
                } else {
                    double lsum = 0.0;
                    double rsum = 0.0;
                    double lw   = 0.0;
                    ...
                    for (uint16_t ch = 0; ch < fmt.channels; ++ch) {
                        const float v =
                            sample_to_float(frame + static_cast<std::size_t>(ch) * bytes_per_sample,
        ...
        const double step =
            static_cast<double>(fmt.sample_rate) / static_cast<double>(kTargetSampleRate);

        while (pos + 1.0 < static_cast<double>(frames)) {
            const auto i = static_cast<std::size_t>(pos);
            const auto a = static_cast<float>(pos - static_cast<double>(i));
...
ExternalAudioSource::~ExternalAudioSource() { shutdown(); }

bool ExternalAudioSource::initialize() { return true; }
    ...
    std::vector<int16_t> chunk;
    {
        ...
        if (to_write > 0) {
            chunk.assign(pcm_queue_.begin() + static_cast<std::ptrdiff_t>(queue_offset_),
                         pcm_queue_.begin() +
                             static_cast<std::ptrdiff_t>(queue_offset_ + to_write));
...
void ExternalAudioSource::compact_queue_locked() {
    if (queue_offset_ > 0 && (queue_offset_ >= pcm_queue_.size() / 2u || queue_offset_ > 32768u)) {
...
void ExternalAudioSource::capture_loop() noexcept {
    ...
    log::info(
        "[external_audio] capturing {} ch, {} Hz, {}-bit container / {}-bit valid {} from '{}'",
    ...
    while (!had_error && !stop_requested_.load(std::memory_order_acquire)) {
        ...
        while (next_packet_frames > 0 && !had_error &&
